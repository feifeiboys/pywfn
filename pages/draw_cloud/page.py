...
class Page:
    def __init__(self,program) -> None:
        ...
        self.window.title(f'渲染点云图{os.path.basename(program.log_path)}')
        self.init_variable()
        self.init_component()
    ...
    def init_component(self): # 定义tkinter组件
        tk.Label(self.window, text='输入原子编号，用,和-分割:').place(x=0, y=100, anchor='nw')
        self.entry1 = tk.Entry(self.window, show=None, width=200)
        tk.Label(self.window, text='输入轨道编号，用,和-分割:').place(x=0, y=200, anchor='w')
        self.entry2 = tk.Entry(self.window, show=None, width=200)
        self.caculate_button = ttk.Button(self.window, text='渲染', command=lambda:threading.Thread(target=self.render).start(),bootstyle=(SECONDARY,OUTLINE))
